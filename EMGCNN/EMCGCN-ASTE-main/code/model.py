...
class EMCGCN(torch.nn.Module):
    def __init__(self, args):
        ...
        self.W = nn.Linear(args.bert_feature_dim*2, args.bert_feature_dim)
        for i in range(self.num_layers):
            self.gcn_layers.append(
                GraphConvLayer(args.device, args.gcn_dim, 5*args.class_num, args.class_num, args.pooling))
        self.SynFue = Encoder.SynFueEncoder(args)

    # def forward(self, tokens, masks, word_pair_position, word_pair_deprel, word_pair_pos, word_pair_synpost):
    def forward(self, tokens, masks, word_pair_position, word_pair_deprel, word_pair_pos,
                word_pair_synpost, simple_word_pair_deprel, word_postag):
