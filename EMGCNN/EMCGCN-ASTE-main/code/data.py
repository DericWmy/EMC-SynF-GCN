...
class Instance(object):
    def __init__(self, tokenizer, sentence_pack, post_vocab, deprel_vocab, postag_vocab, synpost_vocab, postag_ca_vocab, args):
        ...
        """ generate simple word-pair deprel graph"""
        """ 依赖邻接矩阵，只关注两个单词之间是否在依赖树上相邻，而不关注依赖类型"""
        self.simple_word_pair_deprel = torch.zeros(args.max_sequence_len, args.max_sequence_len).long()
        for i in range(len(self.tokens)):
            start = self.token_range[i][0]
            end = self.token_range[i][1]
            for j in range(start, end + 1):
                s, e = self.token_range[self.head[i] - 1] if self.head[i] != 0 else (0, 0)
                for k in range(s, e + 1):
                    self.simple_word_pair_deprel[j][k] = 1
                    self.simple_word_pair_deprel[k][j] = 1
                    self.simple_word_pair_deprel[j][j] = 1

        """generate postag index of the words in sentences"""
        self.word_postag = torch.zeros(args.max_sequence_len).long()
        for i in range(len(self.tokens)):
            self.word_postag[i] = postag_ca_vocab.stoi.get(self.postag[i])
...
def load_data_instances(sentence_packs, post_vocab, deprel_vocab, postag_vocab, synpost_vocab, postag_ca_vocab, args):
    instances = list()
    tokenizer = BertTokenizer.from_pretrained(args.bert_model_path)
    for sentence_pack in sentence_packs:
        instances.append(Instance(tokenizer, sentence_pack, post_vocab, deprel_vocab, postag_vocab, synpost_vocab, postag_ca_vocab, args))
    return instances
...
class DataIterator(object):
    ...
    def get_batch(self, index):
        ...
        word_pair_position = []
        word_pair_deprel = []
        word_pair_pos = [] # postag
        word_pair_synpost = []
        # 把邻接矩阵提取出来
        simple_word_pair_deprel = []
        word_postag = []
        ...
        for i in range(index * self.args.batch_size,
            ...
            simple_word_pair_deprel.append(self.instances[i].simple_word_pair_deprel)
            word_postag.append(self.instances[i].word_postag)
        ...
        simple_word_pair_deprel = torch.stack(simple_word_pair_deprel).to(self.args.device)
        word_postag = torch.stack(word_postag).to(self.args.device)
        ...
        return sentence_ids, sentences, bert_tokens, lengths, masks, sens_lens, token_ranges, aspect_tags, tags, \
               word_pair_position, word_pair_deprel, word_pair_pos, word_pair_synpost, tags_symmetry, simple_word_pair_deprel, word_postag